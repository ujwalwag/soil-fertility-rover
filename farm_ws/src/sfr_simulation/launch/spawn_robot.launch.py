...
def get_robot_description(context):
    ...
    _src_farm_desc = os.path.abspath(os.path.join(_launch_dir, "..", "..", "sfr_description"))
    _src_urdf = os.path.join(_src_farm_desc, "urdf", "ugvbeast_arm.urdf")

    urdf_path = os.path.join(pkg_sfr_description, "urdf", "ugvbeast_arm.urdf")
    ...
    if use_ugv:
        ...
        if os.path.exists(_src_urdf):
            urdf_path = _src_urdf
            pkg_sfr_description = _src_farm_desc
        elif not os.path.exists(urdf_path):
            use_ugv = False

    if not use_ugv:
        urdf_path = os.path.join(
            pkg_sfr_description, "urdf", "turtlebot3_om",
            "turtlebot3_waffle_for_open_manipulator.urdf"
        )
    controller_config_path = os.path.join(pkg_sfr_simulation, "config", "all_controllers.yaml")

    with open(urdf_path, "r") as f:
        robot_desc = f.read()

    meshes_ugv = os.path.abspath(os.path.join(pkg_sfr_description, "meshes", "ugv_beast"))
    meshes_arm = os.path.abspath(os.path.join(pkg_sfr_description, "meshes", "turtlebot3_om"))
    ...
    robot_desc = robot_desc.replace(
        "$(find sfr_simulation)/config/all_controllers.yaml", controller_config_path
    )
